...
EventsPerDay <- function(events,station_name,script_file){
  SCRIPT_DIRECTORY <- dirname(script_file)
  DESTDIR <- paste(SCRIPT_DIRECTORY, '../web/static/img/events',sep='/')

  events=na.omit(events)  #remove rows with NA
  events=events[which(as.numeric(events[,4])>=6 & as.numeric(events[,4])<=20),]
  number_of_events_per_day=c()
  ensembles=unique(events[,1])
  days=seq(from = as.Date("2022-08-01"), to = Sys.Date() , by = 'day')
  for(i in 1:length(ensembles)){
        # all events for this "ensemble"
        events_ensemble=events[which(events[,1]==ensembles[i]),]

        number_of_events_per_day_ensemble=c()
        for(i in 1:length(days)){
            number_of_events_per_day_ensemble[i]=length(which(substr(events_ensemble[,2],1,10)==days[i]))
        }
        number_of_events_per_day=cbind(number_of_events_per_day,number_of_events_per_day_ensemble)
  }

  number_of_events_per_day=data.frame(days,rowMeans(number_of_events_per_day))
  ...
  plot(as.POSIXct(number_of_events_per_day[,1]),number_of_events_per_day[,2],cex.lab=1.1,cex.axis=1.1,col="black",las=1,,type='l',xlab="Date",ylab="Number of events per day")

  dev.off()

  return(number_of_events_per_day)
}
